...
def applyEffect(imgPath:str,save_path:str) -> None:
	...
	try:
		...
		if t=="blur":
			...
			res=img.filter(ImageFilter.MinFilter(getConf("effect","min_filter_size")))
		elif t=="max_filter":
			res=img.filter(ImageFilter.MaxFilter(getConf("effect","max_filter_size")))
		elif t=="smooth":
			res=img.filter(ImageFilter.SMOOTH)
		elif t=="smooth_more":
			res=img.filter(ImageFilter.SMOOTH_MORE)
		elif t=="box_blur":
			res=img.filter(ImageFilter.BoxBlur(radius=getConf("effect","box_blur_radius")))
		elif t=="median_blur":
			res=img.filter(ImageFilter.MedianFilter(getConf("effect","median_blur_size")))
		else:
